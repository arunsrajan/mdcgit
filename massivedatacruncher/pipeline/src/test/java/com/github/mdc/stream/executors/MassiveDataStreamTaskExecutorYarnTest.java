...
public class MassiveDataStreamTaskExecutorYarnTest extends MDCPipelineTestsCommon {
	...
	public void testProcessBlockHDFSIntersection() throws Exception {
		...
		MassiveDataStreamTaskDExecutor mdstde = new MassiveDataStreamTaskExecutorYarn(hdfsurl, js);
		mdstde.setHdfs(hdfs);
		...
		InputStream is = new SnappyInputStream(new BufferedInputStream(hdfs.open(new Path(path))));
	...
	public void testProcessStreamBlockHDFSIntersectionDiff() throws Exception {
		...
		is = new SnappyInputStream(new BufferedInputStream(hdfs.open(new Path(path))));
		List<String> intersectiondata = (List<String>) kryo.readClassAndObject(new Input(is));
		assertEquals(20, intersectiondata.size());
		is.close();
	}
	...
	public void testProcessStreamsBlockHDFSIntersectionDiff() throws Exception {
		...
		InputStream is1 = new SnappyInputStream(new BufferedInputStream(hdfs.open(new Path(path))));
		List<InputStream> istreams1 = Arrays.asList(is1);
		path = mdstde.getIntermediateDataFSFilePath(task2);
		InputStream is2 = new SnappyInputStream(new BufferedInputStream(hdfs.open(new Path(path))));
	...
	public void testProcessJoin() throws Exception {
		...
		InputStream is1 = new SnappyInputStream(new BufferedInputStream(hdfs.open(new Path(path1))));
		String path2 = mdstde.getIntermediateDataFSFilePath(reducebykeytask2);
		InputStream is2 = new SnappyInputStream(new BufferedInputStream(hdfs.open(new Path(path2))));
		...
		mdstde.processJoinLZF(is1, is2, jp, false, false);
		is1.close();
		is2.close();
	...
	public void testProcessLeftOuterJoin() throws Exception {
		...
		LeftOuterJoinPredicate<Tuple2<String, Long>, Tuple2<String, Long>> jp = (Tuple2<String, Long> tup1,
				Tuple2<String, Long> tup2) -> tup1.v1.equals(tup2.v1);
		mdstde.setTask(jointask);
		mdstde.processLeftOuterJoinLZF(is1, is2, jp, false, false);
		is1.close();
		is2.close();
	...
	public void testProcessRightOuterJoin() throws Exception {
		...
		RightOuterJoinPredicate<Tuple2<String, Long>, Tuple2<String, Long>> jp = (Tuple2<String, Long> tup1,
				Tuple2<String, Long> tup2) -> tup1.v1.equals(tup2.v1);
		mdstde.setTask(jointask);
		mdstde.processRightOuterJoinLZF(is2, is1, jp, false, false);
		is1.close();
		is2.close();
