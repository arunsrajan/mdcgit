 ...
 *         statuses.
 */
public class YarnAppmasterMDC extends StaticEventingAppmaster implements ContainerLauncherInterceptor {

	private static final Log log = LogFactory.getLog(YarnAppmasterMDC.class);
	...
	private SimpleDirectedGraph<MassiveDataStreamTaskSchedulerThread, DAGEdge> graph = new SimpleDirectedGraph<>(
			DAGEdge.class);
	private Map<String, MassiveDataStreamTaskSchedulerThread> taskmdsthread;
	...
	public void submitApplication() {
		try {
			...
			mdststs = (List<MassiveDataStreamTaskSchedulerThread>) RemoteDataFetcher
			...
			graph = (SimpleDirectedGraph<MassiveDataStreamTaskSchedulerThread, DAGEdge>) RemoteDataFetcher
					.readYarnAppmasterServiceDataFromDFS(yarninputfolder,
							MDCConstants.MASSIVEDATA_YARNINPUT_GRAPH_FILE);
			// task map.
			taskmdsthread = (Map<String, MassiveDataStreamTaskSchedulerThread>) RemoteDataFetcher
			...
			tasks = mdststs.stream().map(MassiveDataStreamTaskSchedulerThread::getTask).collect(Collectors.toList());
			log.debug("tasks size:" + tasks.size());
		} catch (Exception ex) {
			log.debug("Submit Application Error, See cause below \n", ex);
		}
		var appmasterservice = (YarnAppmasterServiceMDC) getAppmasterService();
