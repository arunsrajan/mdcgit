...
/**
 * 
 * @author arun Utils for adding the shutdown hook and obtaining the shuffled task executors and
...
public class Utils {
  ...
  /**
   * This method configures the log4j properties and obtains the properties from the config folder
   * in the binary distribution.
   * 
   * @param propertyfile
   * @throws Exception
   */
  public static void loadLog4JSystemProperties(String propertiesfilepath, String propertyfile)
      throws Exception {
  ...
  /**
   * This method configures the log4j properties and obtains the properties from the classpath in
   * the binary distribution. This method is for testing purposes.
   * 
   * @param propertyfile
   * @throws Exception
   */
  public static void loadLog4JSystemPropertiesClassPath(String propertyfile) throws Exception {
    ...
    PropertyConfigurator.configure(Utils.class
        .getResourceAsStream(MDCConstants.FORWARD_SLASH + MDCConstants.LOG4J_PROPERTIES));
    var prop = new Properties();
    try {
  ...
  /**
   * This method configures the log4j properties and obtains the properties from the classpath in
   * the binary distribution for mesos.
   * 
   * @param propertyfile
   * @throws Exception
   */
  ...
  /**
   * This function creates and configures the jgroups channel object for the given input and returns
   * it. This is used in jgroups mode of autonomous task execution with no scheduler behind it.
   * 
   * @param jobid
  ...
  public static JChannel getChannelTaskExecutor(String jobid, String networkaddress, int port,
      Map<String, WhoIsResponse.STATUS> mapreq, Map<String, WhoIsResponse.STATUS> mapresp)
      throws Exception {
  ...
  /**
   * Request the status of the stage whoever is the executing the stage tasks. This method is used
  ...
  /**
   * Request the status of the all the stages whoever are the executing the stage tasks. This method
   * is used by the job scheduler in jgroups mode of stage task executions.
   * 
   * @param channel
   * @throws Exception
   */
   ...
   * @throws Exception
   */
  public static void whoareresponse(JChannel channel, Address address,
      Map<String, WhoIsResponse.STATUS> maptosend) throws Exception {
  ...
  /**
   * Response of the whois request used by the task executors to execute the next stage tasks.
   * 
   * @param msg
   * @param stagepartitionid
   * @param jobid
   ...
   * @throws Exception
   */
  public static void whoisresp(Message msg, String stagepartitionid, String jobid,
      WhoIsResponse.STATUS status, JChannel jchannel, String networkaddress) throws Exception {
   ...
   */
  public static void renderGraphStage(Graph<Stage, DAGEdge> graph, Writer writer)
      throws ExportException {
    ...
    GraphExporter<Stage, DAGEdge> exporter =
        new DOTExporter<>(vertexIdProvider, vertexLabelProvider, null);
    exporter.exportGraph(graph, writer);
    var path = MDCProperties.get().getProperty(MDCConstants.GRAPDIRPATH);
    new File(path).mkdirs();
    try (var stagegraphfile =
        new FileWriter(path + MDCProperties.get().getProperty(MDCConstants.GRAPHFILESTAGESPLANNAME)
  ...
  public static void renderGraphPhysicalExecPlan(Graph<Task, DAGEdge> graph, Writer writer)
    ...
    new File(path).mkdirs();
    try (var stagegraphfile =
        new FileWriter(path + MDCProperties.get().getProperty(MDCConstants.GRAPHFILEPEPLANNAME)
            + System.currentTimeMillis());) {
      stagegraphfile.write(writer.toString());
    } catch (Exception e) {
      log.error("File Write Error, see cause below \n", e);
  ...
  /**
   * This function returns the object by socket using the host port of the server and the input
  ...
  public static Object getResultObjectByInput(String hp, Object inputobj) throws Exception {
    ...
    try {
      final Registry registry =
          LocateRegistry.getRegistry(hostport[0], Integer.parseInt(hostport[1]));
  ...
  public static synchronized JChannel getChannelTSSHA(String bindaddress, Receiver receiver)
      throws Exception {
    JChannel channel = getChannelWithPStack(bindaddress);
    if (!Objects.isNull(channel)) {
  ...
  public static void createJar(File folder, String outputfolder, String outjarfilename) {
    ...
    try (var target = new JarOutputStream(
        new FileOutputStream(outputfolder + MDCConstants.FORWARD_SLASH + outjarfilename),
        manifest);) {
      add(folder, target);
  ...
  public static ServerCnxnFactory startZookeeperServer(int clientport, int numconnections,
      int ticktime) throws Exception {
    ...
    ServerCnxnFactory scf =
        ServerCnxnFactory.createFactory(new InetSocketAddress(clientport), numconnections);
    scf.startup(server);
    return scf;
  }
  ...
  public static Long getTotalAvailablePhysicalMemory() {
    ...
    if (nonNull(MDCProperties.get().get(MDCConstants.MEMORY_FROM_NODE))) {
      Long memoryavailableinmb =
          Long.valueOf((String) MDCProperties.get().getProperty(MDCConstants.MEMORY_FROM_NODE,
              MDCConstants.MEMORY_FROM_NODE_DEFAULT)) * MDCConstants.MB;
  ...
  public static int getAvailableProcessors() {
    ...
    if (nonNull(MDCProperties.get().get(MDCConstants.CPU_FROM_NODE))) {
      int availableprocessors = Integer.valueOf((String) MDCProperties.get()
          .getProperty(MDCConstants.CPU_FROM_NODE, MDCConstants.CPU_FROM_NODE_DEFAULT));
  ...
  public static void writeResultToHDFS(String hdfsurl, String filepath, InputStream is)
      throws Exception {
    try (var hdfs = FileSystem.get(new URI(hdfsurl), new Configuration());
        BufferedWriter bw = new BufferedWriter(new OutputStreamWriter(hdfs.create(
            new Path(hdfsurl + filepath),
  ...
  public static void writeResultToHDFS(String hdfsurl, String filepath, Object out)
      throws Exception {
    try (var hdfs = FileSystem.get(new URI(hdfsurl), new Configuration());
        BufferedWriter bw =
            new BufferedWriter(new OutputStreamWriter(hdfs.create(new Path(hdfsurl + filepath),
                Short.parseShort(
  ...
  public static String getIntermediateInputStreamRDF(RemoteDataFetch rdf) throws Exception {
    ...
    var path = rdf.getJobid() + MDCConstants.HYPHEN + rdf.getStageid() + MDCConstants.HYPHEN
        + rdf.getTaskid();
  ...
  public static String launchContainers(Integer numberofcontainers) throws Exception {
    ...
    for (int container = 0; container < numavailable; container++) {
      ...
      List<Integer> ports =
          (List<Integer>) Utils.getResultObjectByInput(restolaunch.getNodeport(), ac);
      if (Objects.isNull(ports)) {
        throw new ContainerException("Port Allocation Error From Container");
      }
      log.info(
          "Chamber alloted with node: " + restolaunch.getNodeport() + " amidst ports: " + ports);
      ...
      lc.setJobid(jobid);
      List<Integer> launchedcontainerports =
          (List<Integer>) Utils.getResultObjectByInput(lc.getNodehostport(), lc);
  ...
  public static Registry getRPCRegistry(int port, final StreamDataCruncher streamdatacruncher)
      throws Exception {
    objects.add(streamdatacruncher);
    Registry registry = LocateRegistry.createRegistry(port);
    StreamDataCruncher stub =
        (StreamDataCruncher) UnicastRemoteObject.exportObject(streamdatacruncher, port);
  ...
  public static ServerSocket createSSLServerSocket(int port) throws Exception {
    ...
    ks.load(new FileInputStream(MDCProperties.get().getProperty(MDCConstants.MDC_JKS)),
        password.toCharArray());

    KeyManagerFactory kmf =
        KeyManagerFactory.getInstance(MDCProperties.get().getProperty(MDCConstants.MDC_JKS_ALGO));
    kmf.init(ks, password.toCharArray());

    TrustManagerFactory tmf =
        TrustManagerFactory.getInstance(MDCProperties.get().getProperty(MDCConstants.MDC_JKS_ALGO));
    tmf.init(ks);
    ...
    sslserversocket.bind(
        new InetSocketAddress(InetAddress.getByAddress(new byte[] {0x00, 0x00, 0x00, 0x00}), port),
        256);
