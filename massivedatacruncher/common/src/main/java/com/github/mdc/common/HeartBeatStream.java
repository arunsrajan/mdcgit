...
/**
 * 
 * @author Arun The Heartbeat server for messaging between task scheduler and task executors
...
public final class HeartBeatStream implements HeartBeatCloseable {
  ...
  ConcurrentMap hpresmap = new ConcurrentHashMap<>();
  ...
  public void start() throws Exception {
    ...
    if (clusterid != null && !clusterid.trim().equals(MDCConstants.EMPTY)) {
      ...
      channel.connect(MDCConstants.TSS + MDCConstants.HYPHEN
  ...
  /**
   * Ping the resource information availability in task executor to task scheduler heartbeat server
   */
  @Override
  public void ping() throws Exception {
    ...
    if (clusterid != null && !clusterid.trim().equals(MDCConstants.EMPTY)) {
      ...
      resources.setNumberofprocessors(Utils.getAvailableProcessors());
      ...
      channel.setReceiver(new Receiver() {
        public void receive(Message msg) {
          if (msg.getObject() instanceof ResponseReceived rr
              && resources.getNodeport().equals(rr.getHp())) {
