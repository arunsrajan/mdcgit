        ...
        for (int numport = 0; numport < ac.getNumberofcontainers(); numport++) {
          while (true) {
            try (ServerSocket s = new ServerSocket(0);) {
              ...
              if (port + MDCConstants.PORT_OFFSET > 65535) {
                log.info("port {} in use, so getting port again...",
                    port + MDCConstants.PORT_OFFSET);
                continue;
              }
              try (var sock =
                  new Socket((String) MDCProperties.get().get(MDCConstants.TASKEXECUTOR_HOST),
                      port + MDCConstants.PORT_OFFSET)) {
                log.info("port {} in use, so getting port again...",
                    port + MDCConstants.PORT_OFFSET);
                continue;
              } catch (Exception ex) {
                log.error("port is not in use, can use port {}", port);
              }
        ...
        if (!Objects.isNull(processes)) {
          ...
          containerprocesses.keySet().stream().forEach(key -> {
            containerprocesses.get(key).keySet().stream()
                .filter(port -> port.equals(dc.getContainerhp().split(MDCConstants.UNDERSCORE)[1]))
                .forEach(port -> {
  ...
  public void destroyProcess(String port, Process proc) {
    try {
      ...
      log.info("Initiated eradicating the chamber case: {}",
          MDCProperties.get().getProperty(MDCConstants.TASKEXECUTOR_HOST) + MDCConstants.UNDERSCORE
              + port);
