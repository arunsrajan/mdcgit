...
public class TaskExecutorReducer implements Callable<Context> {
  ...
  public TaskExecutorReducer(ReducerValues rv, String applicationid, String taskid, ClassLoader cl,
      int port, Map<String, Object> apptaskexecutormap) throws Exception {
    this.rv = rv;
    Class<?> clz = null;
    this.port = port;
    try {
  ...
  public Context call() {
    ...
    try {
      ...
      for (var tuple3 : (List<Tuple3>) rv.getTuples()) {
          ...
          if (apptaskcontextmap.get(apptaskids) != null) {
            ...
            if (temc == null) {
              ...
              currentctx = (Context) Utils
                  .getResultObjectByInput((String) ((List) tuple3.v3).get(hpcount), objects);
            } else {
        ...
        var mdcr = new ReducerExecutor((DataCruncherContext) ctx, cr, tuple3.v1);
