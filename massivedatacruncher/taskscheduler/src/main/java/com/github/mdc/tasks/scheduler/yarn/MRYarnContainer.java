...
import com.github.mdc.tasks.executor.MassiveDataCruncherMapperCombiner;
 ...
 *         API.
 */
public class MRYarnContainer extends AbstractIntegrationYarnContainer {

	private Map<String, String> containerprops;

	private static final Log log = LogFactory.getLog(MRYarnContainer.class);
			...
			while (true) {
				...
				if (response.getState().equals(JobResponse.State.STANDBY)) {
					...
					if(object instanceof MapperCombiner mc) {
						...
						var cm = new ArrayList<CrunchMapper>();
						var cc = new ArrayList<CrunchCombiner>();
						...
						try (var hdfs = FileSystem.newInstance(
							...
							var mdcmc = new MassiveDataCruncherMapperCombiner(
					...
					}else if(object instanceof Reducer red) {
						Class<?> clz = null;
						clz = getClass().getClassLoader().loadClass(red.reducerclasses.iterator().next());
						var cr = (CrunchReducer) clz.newInstance();
						...
						for (var tuple2 : (List<Tuple2>)red.tuples) {
							...
							var mdcr = new MassiveDataCruncherReducer((DataCruncherContext) ctx, cr,
									tuple2.v1);
