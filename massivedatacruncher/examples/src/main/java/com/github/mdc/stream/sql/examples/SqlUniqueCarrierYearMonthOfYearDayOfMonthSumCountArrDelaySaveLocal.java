...
public class SqlUniqueCarrierYearMonthOfYearDayOfMonthSumCountArrDelaySaveLocal implements Serializable, Pipeline {
	...
	private Logger log = Logger.getLogger(SqlUniqueCarrierYearMonthOfYearDayOfMonthSumCountArrDelaySaveLocal.class);
	...
	public void testSql(String[] args, PipelineConfig pipelineconfig) throws Exception {
		...
		String statement = "SELECT AirlineYear,MonthOfYear,DayofMonth,UniqueCarrier,sum(ArrDelay),count(ArrDelay) "
				+ "FROM airline where ArrDelay<>'NA' and ArrDelay<>'ArrDelay' group by AirlineYear,MonthOfYear,DayofMonth,UniqueCarrier";
