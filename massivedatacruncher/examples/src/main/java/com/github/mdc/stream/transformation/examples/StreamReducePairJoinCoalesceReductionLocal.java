...
public class StreamReducePairJoinCoalesceReductionLocal implements Serializable, Pipeline {
	private static final long serialVersionUID = -7001849661976107123L;
	private Logger log = Logger.getLogger(StreamReducePairJoinCoalesceReductionLocal.class);
	...
	public void testReduce(String[] args, PipelineConfig pipelineconfig) throws Exception {
		log.info("StreamReducePairJoinCoalesceReduction.testReduce Before---------------------------------------");
		...
		carriers.join(airlinesamples)
				.saveAsTextFile(new URI(args[0]), args[3] + "/StreamReducePairJoin-" + System.currentTimeMillis());
		log.info("StreamReducePairJoinCoalesceReduction.testReduce After---------------------------------------");
	}
}
